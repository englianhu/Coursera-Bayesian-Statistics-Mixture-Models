...
```{r error=TRUE}
x <- read.csv(file = "data/fuses.csv", header = FALSE)$V1
KK = 2
n = NROW(x)
...
mean    = mean(x)  #Random cluster centers randomly spread over the support of the data
lambda = 20/mean(x)
mu     =  mean(log(x))
tau    =  sd(log(x))
...
while(!sw){
  ...
  yy = w*dexp(xx, lambda) + (1-w)*dlnorm(xx, mu,tau)
  plot(xx, yy, type="l", ylim=c(0, max(yy)), main=paste("s =",s,"   Q =", round(QQ.out[s],4)), lwd=2, col="red", lty=2, xlab="x", ylab="Density")

  legend(6,0.22,c("EM Estimate"),col="red", lty=1)
}
```
