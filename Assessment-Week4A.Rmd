...
suppressAll(lib('EDISON', 'extraDistr', 'MCMCpack'))
```
...
sigma = sd(x)/KK
cc    = base::sample(1:KK, size=n, replace=TRUE, prob=w)
...
for(s in 1:rrr){
  ...
  for(i in 1:n){
    ...
    cc[i] = base::sample(1:KK, 1, replace=TRUE, prob=v)
  }
...
for(i in 1:n){
  ...
  v    = exp(v - max(v))/sum(exp(v - max(v)))
  cc[i] = base::sample(1:2, 1, replace=TRUE, prob=v)
}
```
...
sigma = rep(sd(x)/2,2)
cc    = base::sample(1:2, n, replace=T, prob=c(1/2,1/2))
...
mu    = rnorm(KK, mean(x), sd(x))
sigma = sd(x)/KK
cc    = base::sample(1:KK, n, replace=T, prob=w)
...
plot(xx, density.mcmc.m, type="n",ylim=c(0,max(density.mcmc.uq)),xlab="eruptions", ylab="Density") polygon(c(xx,rev(xx)), c(density.mcmc.lq, rev(density.mcmc.uq)), col="grey", border="grey")
