...
subtitle: "<span style='color:white; background-color:#4E79A7;'>Bayesian Statistics: Mixture Models</span> (Assessment Week 4B with Codes)"
...
課題の提出期限は、`6月14日 15:59 JST`ですが、可能であれば1日か2日早く提出してください。 早い段階で提出すると、他の受講生のレビューを時間内に得る可能性が高くなります。
...
The classification is based on the probabilities that each observation is classified as "genuine" and "counterfeit". Those probabilities can be estimated using the frequencies for which each $c_i$ is equal to each of the categories:
...
Sigma[1,,] = var(x[banknote.training.labels=="counterfeit",])
Sigma[2,,] = var(x[banknote.training.labels=="genuine",])
cc      = array(0,n+m)
